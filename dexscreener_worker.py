import os import requests import time from datetime import datetime, timezone from dotenv import load_dotenv
...
def send_telegram_alert(message): url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage" payload = { "chat_id": TELEGRAM_CHAT_ID, "text": message, "parse_mode": "HTML" } try: response = requests.post(url, json=payload) print("Telegram status:", response.status_code) print("Response:", response.text) return response.status_code == 200 except Exception as e: print(f"Telegram send failed: {e}") return False

def token_is_safe(token_address): try: resp = requests.get(f"{BIRDEYE_BASE_URL}{token_address}/holders", headers=HEADERS) data = resp.json() if not data.get("data"): return False top_holders = data["data"][:5] for holder in top_holders: if holder.get("share", 0) > MAX_HOLDER_PERCENTAGE: print(f"❌ Top holder share too high: {holder['share']}%") return False return True except Exception as e: print(f"Error in token_is_safe: {e}") return False

def check_dexscreener(): print(f"\n🕵️ {datetime.utcnow()} - Scanning Solana tokens...\n") try: response = requests.get(DEXSCREENER_API_URL) pairs = response.json().get("pairs", [])
...
startup_msg = ( "✅ <b>Bot started and ready to snipe</b>\n" "<i>Monitoring Solana tokens every 5 minutes with LP lock, top holders ≤ 5% and min $10k liquidity</i>" ) send_telegram_alert(startup_msg)

🔁 Run loop

if name == "main": while True: check_dexscreener() time.sleep(300)  # every 5 minutes
