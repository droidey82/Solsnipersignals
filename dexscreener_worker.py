...
MIN_LIQUIDITY_USD = 10000 MIN_VOLUME_5M = 15000 MAX_HOLDER_PERCENTAGE = 5 MIN_VOLUME_SPIKE_RATIO = 1.2  # 20% spike
...
def is_lp_locked_or_burned(token): tags = token.get("tags", []) tags_text = ",".join(tags).lower() return "locked" in tags_text or "burned" in tags_text
...
for token in pairs:
        ...
        volume_now = float(token.get("volume", {}).get("h5", 0))
        volume_prev = float(token.get("volume", {}).get("h1", 0))

        if liquidity < MIN_LIQUIDITY_USD or volume_now < MIN_VOLUME_5M:
            continue

        if volume_prev > 0 and (volume_now / volume_prev) < MIN_VOLUME_SPIKE_RATIO:
            continue

        if not is_lp_locked_or_burned(token):
            continue
