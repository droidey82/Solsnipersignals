...
MIN_LIQUIDITY_USD = 10000 MIN_VOLUME_5M = 15000 MAX_HOLDER_PERCENTAGE = 5 VOLUME_SPIKE_THRESHOLD = 1.2  # 20% increase

SEEN_TOKENS = set()

def send_telegram_alert(message): url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage" payload = { "chat_id": TELEGRAM_CHAT_ID, "text": message, "parse_mode": "HTML" } try: response = requests.post(url, json=payload) print(f"Sent alert. Telegram response: {response.text}") return response.status_code == 200 except Exception as e: print(f"Telegram send failed: {e}") return False
...
def check_dexscreener(): try: print(f"[{datetime.now(timezone.utc)}] Scanning Solana pairs...") response = requests.get(DEXSCREENER_API_URL) pairs = response.json().get("pairs", [])
...
startup_msg = ( "\u2705 Bot started and ready to snipe\n" "<i>Monitoring Solana tokens every 5 minutes with LP lock, top holders \u2264 5% and min $10k liquidity</i>" ) send_telegram_alert(startup_msg) print("\u2705 Startup alert sent to Telegram.")
